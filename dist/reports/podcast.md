# 播客源检测报告

生成时间: 2026-08-13T19:59:02.853Z

活跃规则: 仅保留最近 1 个月内有音频条目更新的 RSS/Atom 源。

总数: 109
可用: 51
失败: 58

## 失败项

1. 声东击西
   地址:
   ```text
   https://feeds.fireside.fm/shengdongjixi/rss
   ```
   错误: This operation was aborted

2. 声动早咖啡
   地址:
   ```text
   https://feeds.fireside.fm/sheng-espresso/rss
   ```
   错误: This operation was aborted

3. What's Next 科技早知道
   地址:
   ```text
   https://feeds.fireside.fm/guiguzaozhidao/rss
   ```
   错误: This operation was aborted

4. 一天世界
   地址:
   ```text
   https://yitianshijie.net/episodes/feed.xml
   ```
   错误: This operation was aborted

5. 内核恐慌
   地址:
   ```text
   https://pan.icu/feed
   ```
   错误: This operation was aborted

6. 比特新声
   地址:
   ```text
   https://bitvoice.banlan.show/feed.xml
   ```
   错误: This operation was aborted

7. 博物志
   地址:
   ```text
   https://bowuzhi.fm/feed/audio.xml
   ```
   错误: This operation was aborted

8. 太医来了
   地址:
   ```text
   https://taiyilaile.com/episodes/feed.xml
   ```
   错误: This operation was aborted

9. UX Coffee 设计咖
   地址:
   ```text
   https://uxcoffee.typlog.io/episodes/feed.xml
   ```
   错误: This operation was aborted

10. 疯投圈
   地址:
   ```text
   https://crazy.capital/feed
   ```
   错误: This operation was aborted

11. 迟早更新
   地址:
   ```text
   https://podcast.weareones.com/feed/audio.xml
   ```
   错误: This operation was aborted

12. 正经不良人
   地址:
   ```text
   https://media.rss.com/zjblrfm/feed.xml
   ```
   错误: This operation was aborted

13. 忽左忽右中国版
   地址:
   ```text
   https://www.ximalaya.com/album/12817863.xml
   ```
   错误: This operation was aborted

14. 创业内幕 Startup Insider
   地址:
   ```text
   https://www.ximalaya.com/album/20119986.xml
   ```
   错误: This operation was aborted

15. 醉英超
   地址:
   ```text
   https://rss.lizhi.fm/rss/385752.xml
   ```
   错误: This operation was aborted

16. 大内密谈
   地址:
   ```text
   https://rss.lizhi.fm/rss/14275.xml
   ```
   错误: This operation was aborted

17. 所建所闻
   地址:
   ```text
   https://feeds.fireside.fm/architalk/rss
   ```
   错误: This operation was aborted

18. 瞎说设计
   地址:
   ```text
   https://feeds.soundon.fm/podcasts/0aa385de-e5e8-4125-bcc4-64130c11d5a2.xml
   ```
   错误: This operation was aborted

19. 設計遊牧 Design Nomads
   地址:
   ```text
   https://open.firstory.me/rss/user/cke01v251c1r00803qbohw954
   ```
   错误: This operation was aborted

20. 半拿铁 | 商业沉浮录
   地址:
   ```text
   https://proxy.wavpub.com/caffebreve.xml
   ```
   错误: This operation was aborted

21. 商业就是这样
   地址:
   ```text
   https://feeds.fireside.fm/thatisbiz/rss
   ```
   错误: This operation was aborted

22. 起朱楼宴宾客
   地址:
   ```text
   https://feed.xyzfm.space/ahng8d9qlywl
   ```
   错误: This operation was aborted

23. 乱翻书
   地址:
   ```text
   https://feed.xyzfm.space/yxuruh3f9mc4
   ```
   错误: This operation was aborted

24. 井户端会议
   地址:
   ```text
   https://feed.xyzfm.space/68acw4ke4frl
   ```
   错误: This operation was aborted

25. 晚点聊 LateTalk
   地址:
   ```text
   https://feeds.fireside.fm/latetalk/rss
   ```
   错误: This operation was aborted

26. 硅谷101
   地址:
   ```text
   https://feeds.fireside.fm/sv101/rss
   ```
   错误: This operation was aborted

27. 随机波动 StochasticVolatility
   地址:
   ```text
   https://feeds.fireside.fm/stovol/rss
   ```
   错误: This operation was aborted

28. 边角聊
   地址:
   ```text
   https://feeds.fireside.fm/leftovertalk/rss
   ```
   错误: This operation was aborted

29. 一知半解
   地址:
   ```text
   https://feeds.podshow.cn/smatter.xml
   ```
   错误: This operation was aborted

30. 津津乐道
   地址:
   ```text
   https://feeds.jjldbk.com/all.xml
   ```
   错误: This operation was aborted

31. Teahour
   地址:
   ```text
   https://feeds.fireside.fm/teahour/rss
   ```
   错误: This operation was aborted

32. 捕蛇者说
   地址:
   ```text
   https://pythonhunter.org/episodes/feed.xml
   ```
   错误: This operation was aborted

33. 枫言枫语
   地址:
   ```text
   https://justinyan.me/feed/podcast
   ```
   错误: HTTP 403

34. talich 闲侃
   地址:
   ```text
   https://talich.fm/episodes/feed.xml
   ```
   错误: not updated in last month, latest audio update 2026-05-18T05:13:03.000Z

35. 硬影像
   地址:
   ```text
   http://hardimage.pro/episodes/feed.xml
   ```
   错误: not updated in last month, latest audio update 2026-07-06T22:29:17.000Z

36. Anyway.FM
   地址:
   ```text
   https://anyway.fm/rss.xml
   ```
   错误: not RSS/Atom XML

37. 五湖四海 5lake4sea
   地址:
   ```text
   https://5l4s.de/feed/audio.xml
   ```
   错误: not updated in last month, latest audio update 2026-05-26T13:22:36.000Z

38. 出海进行时
   地址:
   ```text
   https://feed.xyzfm.space/cf4yqd4g79lv
   ```
   错误: not updated in last month, latest audio update 2026-07-08T23:30:00.000Z

39. 下楼散步
   地址:
   ```text
   https://feed.xyzfm.space/wxvp6yquudvy
   ```
   错误: not updated in last month, latest audio update 2026-06-09T14:11:53.000Z

40. 尴尬院长·粤语播客
   地址:
   ```text
   http://rss.lizhi.fm/rss/1063597.xml
   ```
   错误: not updated in last month, latest audio update 2026-05-11T04:28:21.000Z

41. 过刊
   地址:
   ```text
   https://www.ximalaya.com/album/36470328.xml
   ```
   错误: not updated in last month, latest audio update 2026-06-13T11:39:40.000Z

42. 剧谈社
   地址:
   ```text
   https://www.ximalaya.com/album/43127335.xml
   ```
   错误: not updated in last month, latest audio update 2026-07-12T16:00:00.000Z

43. 普通读者
   地址:
   ```text
   https://anchor.fm/s/3bd8896c/podcast/rss
   ```
   错误: not updated in last month, latest audio update 2026-07-09T18:15:05.000Z

44. 例外状态 State of Exception
   地址:
   ```text
   https://anchor.fm/s/5d9c1a28/podcast/rss
   ```
   错误: not updated in last month, latest audio update 2026-06-03T05:42:14.000Z

45. 蜜獾吃书
   地址:
   ```text
   https://www.ximalaya.com/album/64689453.xml
   ```
   错误: not updated in last month, latest audio update 2026-07-11T11:59:09.000Z

46. 漂浮绿洲 Floating Oasis
   地址:
   ```text
   https://anchor.fm/s/e60787d0/podcast/rss
   ```
   错误: not updated in last month, latest audio update 2026-05-30T01:51:59.000Z

47. 好青年荼毒室
   地址:
   ```text
   https://anchor.fm/s/3f94b8a0/podcast/rss
   ```
   错误: not updated in last month, latest audio update 2026-07-01T23:00:00.000Z

48. 无人知晓
   地址:
   ```text
   https://feed.xyzfm.space/ypn9dydpbxpc
   ```
   错误: not updated in last month, latest audio update 2026-03-03T12:00:00.000Z

49. 范特西码后炮
   地址:
   ```text
   https://anchor.fm/s/6ade80f4/podcast/rss
   ```
   错误: not updated in last month, latest audio update 2026-06-29T22:45:04.000Z

50. 这刺聊点啥
   地址:
   ```text
   https://www.ximalaya.com/album/29601116.xml
   ```
   错误: not updated in last month, latest audio update 2026-01-06T21:57:56.000Z

51. FView Friday
   地址:
   ```text
   https://www.ximalaya.com/album/41563226.xml
   ```
   错误: not updated in last month, latest audio update 2026-07-11T07:20:06.000Z

52. 嘻谈录
   地址:
   ```text
   https://www.ximalaya.com/album/43044571.xml
   ```
   错误: not updated in last month, latest audio update 2025-07-26T15:30:00.000Z

53. 奇妙店台
   地址:
   ```text
   http://www.ximalaya.com/album/8224043.xml
   ```
   错误: not updated in last month, latest audio update 2025-06-16T01:07:36.000Z

54. Kotlin 炉边漫谈
   地址:
   ```text
   https://www.ximalaya.com/album/68370676.xml
   ```
   错误: not updated in last month, latest audio update 2025-04-07T08:17:21.000Z

55. 夸夸歧谈
   地址:
   ```text
   https://anchor.fm/s/9422c4e8/podcast/rss
   ```
   错误: not updated in last month, latest audio update 2025-06-02T23:25:58.000Z

56. 酒嗝播客
   地址:
   ```text
   http://rss.lizhi.fm/rss/171448272.xml
   ```
   错误: not updated in last month, latest audio update 2025-01-27T03:53:37.000Z

57. 史播客
   地址:
   ```text
   https://feeds.soundon.fm/podcasts/e9ce0c40-9b4a-41e2-a19e-db9e5aeca3f7.xml
   ```
   错误: not updated in last month, latest audio update 2025-01-09T12:41:05.000Z

58. 南方聲活
   地址:
   ```text
   https://feeds.soundon.fm/podcasts/dce49dcf-aadf-43d6-8b24-4e7ed3e37da6.xml
   ```
   错误: not updated in last month, latest audio update 2025-09-08T12:04:03.000Z
